...
const AccounTypePicker = ({ selectedAccountType, setSelectedAccountType }) => {
  const [countries, setCountries] = useState([]);
  const [filteredCountries, setFilteredCountries] = useState([]);
  const [modalVisible, setModalVisible] = useState(false);
  const [loading, setLoading] = useState(true);
  const [selectedAccountTypeName, setSelectedAccountTypeName] = useState('');
  ...
  const handleSelect = (country) => {
    setSelectedAccountTypeName(country.name);
    setSelectedAccountType(country.id);
  ...
  return (
    <View> 
      ...
      <TouchableOpacity 
        ...
        <Text>{selectedAccountType || 'Select Account Type'}</Text>
      </TouchableOpacity>
...
export default AccounTypePicker;
