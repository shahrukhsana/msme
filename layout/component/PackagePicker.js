...
const PackagePicker = ({ selectedPackage, setSelectedPackage, extraData=[] }) => {
  ...
  const fetchPickerData = async () => { 
    try {
      const response = await postData({}, urls.package, "GET", null, extraData);
  ...
  const handleSelect = (country) => {
    setSelectedPackageName(country.name);
    setSelectedPackage(country.id);
  ...
  return (
    <View> 
      ...
      <TouchableOpacity 
        ...
        <Text>{selectedPackageName || 'Select Package'}</Text>
      </TouchableOpacity>
